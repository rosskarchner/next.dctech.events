...
routes/submit.py:_sanitize_redirect_path. It also determines which
magic_link purpose the token gets signed for (submit vs prefs), so a
preferences-link request can't mint a token that doubles as a submission
...
def test_no_redirect_path_falls_back_to_the_submission_form():
    assert _sanitize_redirect_path(None) == ('/edit/submit-event.html', 'submit')
    assert _sanitize_redirect_path('') == ('/edit/submit-event.html', 'submit')
...
def test_a_valid_guid_is_carried_through():
    assert _sanitize_redirect_path('/edit/correct-event.html?guid=abc123ef') == \
        ('/edit/correct-event.html?guid=abc123ef', 'submit')
...
def test_a_full_32_hex_guid_is_carried_through():
    ...
    assert _sanitize_redirect_path(f'/edit/correct-event.html?guid={guid}') == \
        (f'/edit/correct-event.html?guid={guid}', 'submit')
...
def test_a_preferences_redirect_drops_any_query_params():
    # Unlike the correction form, preferences.html has no accepted query
    # param — anything appended is dropped, not treated as an invalid path.
    assert _sanitize_redirect_path('/edit/preferences.html?categories=ai') == \
        ('/edit/preferences.html', 'prefs')
